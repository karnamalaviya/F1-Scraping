...
# 2018 Data
df_2018 <- read.csv("fantasy_data_2018.csv", na = NA, stringsAsFactors = FALSE)

# 2019 Data
df_2019 <- read.csv("fantasy_data_2019.csv", na = NA, stringsAsFactors = FALSE)

# Team Selection Data
team_selection <- read.csv("choices_data_2019.csv", na = NA, stringsAsFactors = FALSE)
    ...
    output$team_selections <- renderPlotly( {
      plot_ly(team_selection, y = ~avg_league_position, z = ~points) %>% 
        ...
        layout(scene = list(xaxis = list(title = 'Price ($ millions)', 
                            ...
                            yaxis = list(title = 'Average Team League Position',
